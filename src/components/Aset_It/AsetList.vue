<script>
import { deleteInventori, useLoadInventori } from '@/firebase/Aset_It/AsetCrud'
...
export default {
  setup() {
    const inventori = useLoadInventori()
    const currentPage = ref(1)
    const pageSize = 3

    const paginatedinventori = computed(() => {
      const start = (currentPage.value - 1) * pageSize
      return inventori.value.slice(start, start + pageSize)
    })

    const totalPages = computed(() => {
      return Math.ceil(inventori.value.length / pageSize)
    })
    ...
    return { inventori, paginatedinventori, currentPage, totalPages, goToPage, hapusInventori, deleteInventori }
  },
}
</script>
...
<template>
  <div class="max-w-xl mx-auto mt-10 p-5 bg-white dark:bg-foreground rounded-xl shadow-md">
    <div class="text-white">
      <p class="text-xl font-bold">List Inventori</p>
      <hr />
    </div>
    <table class="border border-white mt-5">
      <thead>
        <tr class="border border-white text-white">
          ...
          <th class="border border-white p-2">Stok</th>
          <th class="border border-white p-2">Status Barang</th>
          <th class="border border-white p-2">Kategori</th>
          <th class="border border-white p-2">Deskripsi</th>
        </tr>
      </thead>
      <tbody>
        <tr v-for="{ id, tanggal, nama_barang, stok,status_barang, kategori, deskripsi } in paginatedinventori" :key="id" class="text-white text-sm">
          <td class="p-2 border-b-2 border-white">
            <div class="flex gap-2">
              <router-link :to="`/aset-it-edit/${id}`">
                <button class="cursor-pointer">
                  <v-icon name="hi-pencil-alt" scale="1" class="text-green-500" />
                </button>
              </router-link>
              <button class="cursor-pointer" @click="hapusInventori(id)">
                <v-icon name="md-deleteforever" scale="1" class="text-red-500" />
              </button>
            </div>
          </td>
          <td class="p-2 border border-white">{{ tanggal || '-' }}</td>
          <td class="p-2 border border-white">{{ nama_barang }}</td>
          <td class="p-2 border border-white">{{ stok}}</td>
          <td class="p-2 border border-white">{{ status_barang }}</td>
